...
(function () {
  ...
  function ensureGlobalSearchAssets() {
    ...
    if (window.KT_SHELL && typeof window.KT_SHELL.ensureGlobalSearchAssets === "function") {
      window.KT_SHELL.ensureGlobalSearchAssets();
      return;
    }
  ...
  var FALLBACK_NAV = {
    version: 1,
    primary: [
      { type: "page", id: "prominent-figures" },
      { type: "page", id: "major-scientific-inventions" },
      { type: "page", id: "industrial-revolutions" }
    ]
  };

  var FALLBACK_ROUTES = {
    pages: [
      { id: "home", az: "az/index.html", en: "en/index.html" },
      { id: "prominent-figures", az: "az/prominent_figures.html", en: "en/prominent_figures.html" },
      {
      ...
      { id: "prominent-figure", az: "az/prominent_figures.html", en: "en/prominent_figures.html" }
    ]
  };
  ...
  function fallbackPathKey() {
    var path = location.pathname.replace(/\\/g, "/");
    var localeIdx = path.search(/\/(az|en)(\/|$)/i);
    if (localeIdx >= 0) {
      path = path.slice(localeIdx + 1).replace(/^\//, "");
    } else {
      path = path.split("/").pop() || "";
    }
  ...
  function fallbackPageHref(page, lang) {
    ...
    var hereSuffix = fallbackPathKey();
  ...
  function pageHref(page, lang) {
    if (activeI18n && typeof activeI18n.pageHref === "function") {
      return activeI18n.pageHref(page, lang);
    }
    return fallbackPageHref(page, lang);
  }
  ...
  function pageDesc(ui, lang, pageId) {
    var key = PAGE_LABEL_KEYS[pageId];
    if (!key) return "";
    var desc = label(ui, lang, key + "Desc");
    return desc && desc !== key + "Desc" ? desc : "";
  }
  ...
  function currentPageId(routes) {
    var explicit = document.documentElement.getAttribute("data-kt-page-id");
    if (explicit) return explicit;
    if (activeI18n && typeof activeI18n.getPageId === "function") {
  ...
  function isForumNavPageId(id) {
    if (window.KT_NAV && typeof window.KT_NAV.isForumNavPageId === "function") {
      return window.KT_NAV.isForumNavPageId(id);
    }
    return id === "forum-2024" || (typeof id === "string" && id.indexOf("forum-") === 0);
  }

  function linkIsActive(page, activeId) {
    if (!activeId || !page) return false;
    if (page.id === activeId) return true;
    if (page.id === "prominent-figures" && activeId === "prominent-figure") return true;
    return false;
  }
  ...
  function childLinkIsActive(page, childDef, activeId) {
    ...
    if (
      (childDef.id === "prominent-figures" || childDef.id === "encyclopedia") &&
      activeId === "prominent-figure"
    ) {
      return true;
    }
    return false;
  ...
  function buildLink(page, lang, ui, activeId, className, extra) {
    ...
    var desc = pageDesc(ui, lang, page.id);
    if (desc) a.setAttribute("title", desc);
    if (linkIsActive(page, activeId)) {
  ...
  function associatePanel(toggle, panel) {
    if (!panel.id) panel.id = "kt-nav-panel-" + (++navPanelSeq);
  ...
  function applyNavAria(ui, lang) {
    ...
    if (toggle && !toggle.getAttribute("data-kt-menu-labels")) {
      toggle.setAttribute("data-kt-menu-labels", "1");
  ...
  function renderFallbackNav(menu, lang) {
    var routes = FALLBACK_ROUTES;
    var ui = FALLBACK_UI;
    var navDef = FALLBACK_NAV;
    var activeId = document.documentElement.getAttribute("data-kt-page-id") || "";
    while (menu.firstChild) menu.removeChild(menu.firstChild);
    menu.appendChild(buildMenu(navDef, routes, lang, ui, activeId));
    menu.setAttribute("data-kt-nav-ready", "1");
    if (window.KT_NAV && typeof window.KT_NAV.init === "function") {
      window.KT_NAV.init();
    }
    if (window.KT_SHELL && typeof window.KT_SHELL.ensureGlobalSearchAssets === "function") {
      window.KT_SHELL.ensureGlobalSearchAssets();
  ...
  function init(i18nApi) {
    ...
    Promise.all([activeI18n.loadRoutes(), activeI18n.loadUi(), activeI18n.loadNav()])
      .then(function (results) {
        ...
        document.dispatchEvent(new CustomEvent("kt-primary-nav-ready"));
      })
      .catch(function (err) {
        console.error("[kt-primary-nav] Menu build failed:", err);
        renderFallbackNav(menu, lang);
        document.dispatchEvent(new CustomEvent("kt-primary-nav-ready"));
      });
  }

  function boot(attempt) {
    var api = window.KT_I18N;
    if (!api) {
      if (attempt < 40) {
        setTimeout(function () {
          boot(attempt + 1);
        ...
        if (menu) {
          var lang =
            document.documentElement.getAttribute("data-kt-lang") === "en" ? "en" : "az";
          renderFallbackNav(menu, lang);
          document.dispatchEvent(new CustomEvent("kt-primary-nav-ready"));
        }
      }
