...
(function (global) {
  ...
  var STORAGE_KEY = "kt-lang-position";
  ...
  function anchorWeight(el) {
    ...
    if (el.hasAttribute("data-kt-section-id")) return 100;
  ...
  function getLogicalAnchor() {
    ...
    for (var i = 0; i < candidates.length; i++) {
      var el = candidates[i];
      var anchorId = el.id || el.getAttribute("data-kt-section-id");
  ...
  function navOffset() {
    ...
    var h = parseFloat(style.getPropertyValue("--kt-sticky-top-stack"));
    if (!isFinite(h) || h <= 0) {
      h = parseFloat(style.getPropertyValue("--kt-nav-height"));
      if (!isFinite(h) || h <= 0) {
  ...
  function scrollToAnchor(id, smooth) {
    ...
    var el =
      ...
      document.querySelector('[data-kt-section-id="' + id + '"]');
    if (!el) return false;
    var top =
